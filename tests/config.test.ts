...
describe('config.ts', () => {
  ...
  it('解析成功并带默认值', () => {
    ...
    expect(cfg.ecs.poolBootDelayMs).toBe(20000)
    expect(cfg.ecs.poolBootstrapMessageId).toBe('pool-mq-worker')
    expect(cfg.ecs.poolBootstrapDockerDetached).toBe(true)
    expect(cfg.ecs.poolDockerContainer).toBeUndefined()
    expect(cfg.ecs.poolBootCommand).toBeUndefined()
    ...
    expect(cfg.scheduler.scaleDownIdleMs).toBe(180000)
    expect(cfg.scheduler.minRunningInstances).toBe(0)
  })
